...
// In production (Vercel), the backend is on the same domain at /api
// In development, we can still use localhost if running separately, but relative path '/api' 
// works best if using the proxy or Vercel dev.
const BASE_URL = (import.meta as any).env.PROD ? '/api' : 'http://localhost:3001/api';
