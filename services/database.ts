...
export class Database {
  ...
  private static async runGlobalCleanup() { const cutoff = new Date(Date.now() - 15000).toISOString(); await supabase.from('active_sessions').delete().lt('last_ping', cutoff); await supabase.from('session_queue').delete().lt('last_ping', cutoff); }
  ...
  static async getQueuePosition(userId: string): Promise<number> { const { data } = await supabase.from('session_queue').select('created_at').eq('user_id', userId).single(); if (!data) return 0; const { count } = await supabase.from('session_queue').select('*', { count: 'exact', head: true }).lt('created_at', data.created_at); return (count || 0) + 1; }
