...
const SoundscapePlayer: React.FC = () => {
    ...
    const TRACKS = { rain: "https://assets.mixkit.co/active_storage/sfx/2393/2393-preview.mp3", forest: "https://assets.mixkit.co/active_storage/sfx/2434/2434-preview.mp3", white: "https://assets.mixkit.co/active_storage/sfx/2513/2513-preview.mp3", lofi: "https://assets.mixkit.co/active_storage/sfx/1196/1196-preview.mp3" };

    useEffect(() => {
        if (audioRef.current) { audioRef.current.pause(); audioRef.current.src = ""; }
        const audio = new Audio(); audio.src = TRACKS[track as keyof typeof TRACKS]; audio.loop = true; audio.volume = volume;
        audio.onerror = (e) => { console.error("Audio Load Error:", e); setPlaying(false); };
        audioRef.current = audio;
        if (playing) { const playPromise = audio.play(); if (playPromise !== undefined) { playPromise.catch(error => { console.warn("Auto-play blocked:", error); setPlaying(false); }); } }
        return () => { if (audioRef.current) { audioRef.current.pause(); audioRef.current = null; } };
    }, [track]);

    useEffect(() => { if (audioRef.current) audioRef.current.volume = volume; }, [volume]);
    const togglePlay = () => { if (!audioRef.current) return; if (playing) { audioRef.current.pause(); setPlaying(false); } else { const playPromise = audioRef.current.play(); if (playPromise !== undefined) { playPromise.then(() => setPlaying(true)).catch(e => { console.error("Play failed:", e); setPlaying(false); }); } } };
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[80] flex items-center gap-2 bg-yellow-500/90 dark:bg-yellow-600/90 backdrop-blur-md p-2 rounded-full border border-yellow-300 dark:border-yellow-500 shadow-xl transition-all hover:bg-yellow-400 animate-in fade-in slide-in-from-bottom-10">
            <button onClick={togglePlay} className={`p-3 rounded-full transition-colors ${playing ? 'bg-black text-white' : 'bg-white/20 text-black dark:text-white'}`}>{playing ? <Music className="w-4 h-4 animate-pulse" /> : <Play className="w-4 h-4 ml-0.5" />}</button>
            {playing && (
                <div className="flex items-center gap-2 px-2 animate-in zoom-in fade-in duration-300 origin-left">
                    <select value={track} onChange={e => setTrack(e.target.value)} className="bg-transparent text-xs font-bold text-black dark:text-white outline-none w-20 cursor-pointer"><option value="rain">Rain</option><option value="forest">Nature</option><option value="white">Cafe</option><option value="lofi">Ocean</option></select>
                    <input type="range" min="0" max="1" step="0.1" value={volume} onChange={e => setVolume(parseFloat(e.target.value))} className="w-16 h-1 bg-black/20 dark:bg-white/20 rounded-lg appearance-none cursor-pointer accent-black dark:accent-white" />
                </div>
            )}
        </div>
    );
...
const WeatherEffect: React.FC<{ type: 'confetti' | 'rain' }> = ({ type }) => {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < particleCount; i++) {
            particles.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height - canvas.height, vx: type === 'confetti' ? (Math.random() - 0.5) * 10 : 0, vy: type === 'confetti' ? Math.random() * 5 + 2 : Math.random() * 15 + 10, color: type === 'confetti' ? ['#FACC15', '#FFD700', '#FF0000', '#00FF00', '#0000FF'][Math.floor(Math.random() * 5)] : '#60A5FA', size: type === 'confetti' ? Math.random() * 8 + 4 : Math.random() * 2 + 1, length: type === 'rain' ? Math.random() * 20 + 10 : 0 });
        }
        const animate = () => {
            if (!ctx) return; ctx.clearRect(0, 0, canvas.width, canvas.height);
            particles.forEach((p) => { p.x += p.vx; p.y += p.vy; if (type === 'confetti') { p.vy += 0.1; ctx.fillStyle = p.color; ctx.fillRect(p.x, p.y, p.size, p.size); } else { ctx.strokeStyle = p.color; ctx.lineWidth = p.size; ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(p.x, p.y + p.length); ctx.stroke(); } if (p.y > canvas.height) { p.y = -20; p.x = Math.random() * canvas.width; if (type === 'confetti') p.vy = Math.random() * 5 + 2; } });
...
const MindfulMatchGame: React.FC<{ onWin?: () => void }> = ({ onWin }) => {
    const [cards, setCards] = useState<any[]>([]); const [flipped, setFlipped] = useState<number[]>([]); const [solved, setSolved] = useState<number[]>([]); const [won, setWon] = useState(false);
    const ICONS = [Sun, Heart, Music, Zap, Star, Anchor, Feather, Cloud];
    useEffect(() => { initGame(); }, []);
    const initGame = () => { const duplicated = [...ICONS, ...ICONS]; const shuffled = duplicated.sort(() => Math.random() - 0.5).map((icon, i) => ({ id: i, icon })); setCards(shuffled); setFlipped([]); setSolved([]); setWon(false); };
    const handleCardClick = (index: number) => {
        ...
        if (newFlipped.length === 2) { if (cards[newFlipped[0]].icon === cards[newFlipped[1]].icon) { setSolved([...solved, newFlipped[0], newFlipped[1]]); setFlipped([]); } else { setTimeout(() => setFlipped([]), 1000); } }
    };
    ...
    return (
        <div className="bg-gradient-to-br from-yellow-50/50 to-white dark:from-gray-800 dark:to-gray-900 w-full h-full flex flex-col rounded-2xl p-2 border border-yellow-100 dark:border-gray-700 overflow-hidden relative shadow-inner">
            <div className="flex justify-between items-center mb-2 z-10 px-2 pt-1 shrink-0"><h3 className="font-black text-xs md:text-sm text-yellow-900 dark:text-yellow-500 uppercase tracking-widest">Mindful Match</h3><button onClick={initGame} className="p-1 hover:bg-yellow-100 dark:hover:bg-gray-700 rounded-full transition-colors"><RefreshCw className="w-3 h-3 md:w-4 md:h-4 text-yellow-600 dark:text-yellow-400" /></button></div>
            {won ? <div className="flex-1 flex flex-col items-center justify-center animate-in zoom-in"><Trophy className="w-12 h-12 md:w-16 md:h-16 text-yellow-500 mb-2 animate-bounce" /><p className="font-black text-lg md:text-2xl text-yellow-900 dark:text-white">Zen Master!</p><button onClick={initGame} className="mt-4 bg-black dark:bg-white dark:text-black text-white px-4 py-1.5 md:px-6 md:py-2 rounded-full font-bold text-xs md:text-sm hover:scale-105 transition-transform">Replay</button></div> : <div className="flex-1 flex items-center justify-center min-h-0"><div className="grid grid-cols-4 grid-rows-4 gap-1.5 md:gap-2 w-full aspect-square">{cards.map((card, i) => { const isVisible = flipped.includes(i) || solved.includes(i); const Icon = card.icon; return (<div key={i} className="w-full h-full"><button onClick={() => handleCardClick(i)} className={`w-full h-full rounded-md md:rounded-lg flex items-center justify-center transition-all duration-300 ${isVisible ? 'bg-white dark:bg-gray-700 border-2 border-yellow-400 shadow-lg' : 'bg-gray-900 dark:bg-gray-800 shadow-md'}`}>{isVisible && <Icon className="w-4 h-4 md:w-5 md:h-5 text-yellow-500 animate-in zoom-in" />}</button></div>); })}</div></div>}
        </div>
    );
};
...
const CloudHopGame: React.FC = () => {
    // ... (Code remains largely same, just compacted for brevity as no logic changed here)
    const canvasRef = useRef<HTMLCanvasElement>(null); const requestRef = useRef<number | undefined>(undefined); const [score, setScore] = useState(0); const [gameOver, setGameOver] = useState(false); const [gameStarted, setGameStarted] = useState(false); const playerRef = useRef({ x: 150, y: 300, vx: 0, vy: 0 });
    useEffect(() => {
        if (!gameStarted) return; const canvas = canvasRef.current; if (!canvas) return; const ctx = canvas.getContext('2d'); if (!ctx) return; const GRAVITY = 0.4; let platforms = [{x: 0, y: 380, w: 400, h: 40, type: 'ground'}]; let py = 300; while (py > -2000) { platforms.push({ x: Math.random() * 300, y: py, w: 70, h: 15, type: 'cloud' }); py -= 90; }
        const handleKeyDown = (e: KeyboardEvent) => { if (e.key === 'ArrowLeft') playerRef.current.vx = -4; if (e.key === 'ArrowRight') playerRef.current.vx = 4; }; const handleKeyUp = () => { playerRef.current.vx = 0; }; window.addEventListener('keydown', handleKeyDown); window.addEventListener('keyup', handleKeyUp);
        const drawCloud = (x: number, y: number, w: number, h: number) => { ctx.fillStyle = 'white'; ctx.fillRect(x, y, w, h); ctx.beginPath(); ctx.arc(x+10, y, 15, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(x+w-10, y, 15, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(x+w/2, y-5, 20, 0, Math.PI*2); ctx.fill(); };
        const update = () => {
            const p = playerRef.current; p.x += p.vx; if (p.x < -20) p.x = 400; if (p.x > 400) p.x = -20; p.vy += GRAVITY; p.y += p.vy;
            if (p.y < 200) { p.y = 200; setScore(s => s + Math.floor(Math.abs(p.vy))); platforms.forEach(pl => { pl.y += Math.abs(p.vy); if (pl.y > 450) { pl.y = -20; pl.x = Math.random() * 340; } }); }
            if (p.vy > 0) { platforms.forEach(pl => { if (p.y + 20 > pl.y && p.y + 20 < pl.y + 40 && p.x + 20 > pl.x && p.x < pl.x + pl.w) { p.vy = -10; } }); }
            ...
            const grad = ctx.createLinearGradient(0,0,0,400); grad.addColorStop(0,'#38bdf8'); grad.addColorStop(1,'#bae6fd'); ctx.fillStyle = grad; ctx.fillRect(0,0,400,400); ctx.fillStyle = 'white'; for(let i=0; i<20; i++) ctx.fillRect(Math.random()*400, Math.random()*400, 2, 2);
            platforms.forEach(pl => { if(pl.type==='ground') { ctx.fillStyle='#4ade80'; ctx.fillRect(pl.x, pl.y, pl.w, pl.h); } else drawCloud(pl.x, pl.y, pl.w, pl.h); });
    ...
    const handleTap = (e: React.MouseEvent | React.TouchEvent) => { if (!canvasRef.current) return; const clientX = 'touches' in e ? e.touches[0].clientX : (e as React.MouseEvent).clientX; const rect = canvasRef.current.getBoundingClientRect(); if (clientX - rect.left < rect.width / 2) playerRef.current.vx = -5; else playerRef.current.vx = 5; }; const handleRelease = () => { playerRef.current.vx = 0; };
    return (<div className="relative h-full w-full bg-sky-300 overflow-hidden rounded-2xl border-4 border-white dark:border-gray-700 shadow-inner cursor-pointer" onMouseDown={handleTap} onMouseUp={handleRelease} onTouchStart={handleTap} onTouchEnd={handleRelease}><div className="absolute top-2 right-2 bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full font-black text-white text-base md:text-lg z-10">{score}m</div><canvas ref={canvasRef} width={400} height={400} className="w-full h-full object-contain" />{(!gameStarted || gameOver) && (<div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-20"><button onClick={() => { setGameStarted(true); setGameOver(false); setScore(0); playerRef.current = {x:150,y:300,vx:0,vy:0}; }} className="bg-yellow-400 text-yellow-900 px-6 py-2 md:px-8 md:py-3 rounded-full font-black text-sm md:text-lg shadow-xl hover:scale-110 transition-transform flex items-center gap-2"><Play className="w-4 h-4 md:w-5 md:h-5 fill-current" /> {gameOver ? 'Try Again' : 'Play'}</button></div>)}</div>);
};

// --- BREATHING EXERCISE ---
const BreathingExercise: React.FC<{ onClose: () => void; userId: string }> = ({ onClose, userId }) => {
    // ... (Code remains same)
    const [text, setText] = useState("Inhale"); const [timeLeft, setTimeLeft] = useState(120); const [isPlaying, setIsPlaying] = useState(false); const audioRef = useRef<HTMLAudioElement>(null); const [duration, setDuration] = useState(0); const stateRef = useRef({ timeLeft: 120, duration: 0 }); const AMBIENT_SONG_URL = "https://assets.mixkit.co/active_storage/sfx/1196/1196-preview.mp3"; 
    useEffect(() => { if (audioRef.current) { audioRef.current.volume = 0.5; const playPromise = audioRef.current.play(); if (playPromise !== undefined) { playPromise.then(() => setIsPlaying(true)).catch(error => { console.log("Autoplay blocked", error); setIsPlaying(false); }); } } const steps = [{ text: "Inhale", delay: 4000 }, { text: "Hold", delay: 4000 }, { text: "Exhale", delay: 4000 }, { text: "Hold", delay: 4000 }]; let stepIdx = 0; const loop = setInterval(() => { setText(steps[stepIdx].text); stepIdx = (stepIdx + 1) % steps.length; }, 4000); const timer = setInterval(() => { stateRef.current.duration += 1; setDuration(stateRef.current.duration); stateRef.current.timeLeft -= 1; setTimeLeft(stateRef.current.timeLeft); if (stateRef.current.timeLeft <= 0) { Database.recordBreathSession(userId, 120 - stateRef.current.timeLeft + stateRef.current.duration); Database.setBreathingCooldown(Date.now() + 5 * 60 * 1000); clearInterval(loop); clearInterval(timer); onClose(); } }, 1000); return () => { clearInterval(loop); clearInterval(timer); if (audioRef.current) { audioRef.current.pause(); audioRef.current.currentTime = 0; } }; }, []); 
    const manualPlay = () => { if (audioRef.current) { audioRef.current.play().then(() => setIsPlaying(true)); } }; const finishEarly = () => { Database.recordBreathSession(userId, 120 - stateRef.current.timeLeft + stateRef.current.duration); Database.setBreathingCooldown(Date.now() + 5 * 60 * 1000); onClose(); };
    return (<div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] flex items-center justify-center p-4"><audio ref={audioRef} src={AMBIENT_SONG_URL} loop /><div className="relative w-full max-w-md aspect-square flex items-center justify-center flex-col"><button onClick={finishEarly} className="absolute top-0 right-0 text-white/50 hover:text-white"><X className="w-8 h-8" /></button><div className="absolute inset-0 bg-yellow-400/20 dark:bg-yellow-500/20 rounded-full animate-breathe"></div><div className="absolute inset-12 bg-yellow-400/40 dark:bg-yellow-500/40 rounded-full animate-breathe" style={{ animationDelay: '1s' }}></div><div className="relative z-10 text-center text-white"><h2 className="text-4xl font-bold mb-2">{text}</h2><div className="inline-block px-4 py-1 rounded-full bg-white/10 border border-white/20 text-sm font-mono">{Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, '0')}</div>{!isPlaying ? (<button onClick={manualPlay} className="mt-6 flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-full text-xs font-bold transition-colors"><Play className="w-3 h-3 fill-current" /> Tap to Start Music</button>) : (<p className="mt-6 text-xs text-white/50 flex items-center justify-center gap-2 animate-pulse"><Music className="w-3 h-3"/> Calming Audio Active</p>)}</div></div></div>);
};
    ...
    return (
        <div className="bg-[#FFFBEB] dark:bg-gray-900 border border-yellow-200 dark:border-gray-800 p-4 md:p-6 rounded-3xl mb-4 hover:shadow-md transition-all">
            ...
            <div className="flex gap-2">
                <button onClick={() => onMoodSelect('confetti')} className="flex-1 py-3 bg-yellow-100 dark:bg-yellow-900/30 hover:bg-yellow-300 rounded-xl flex items-center justify-center transition-colors"><Sun className="w-5 h-5 md:w-6 md:h-6 text-yellow-600 dark:text-yellow-400" /></button>
                ...
                <button onClick={() => onMoodSelect('rain')} className="flex-1 py-3 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-300 rounded-xl flex items-center justify-center transition-colors"><CloudRain className="w-5 h-5 md:w-6 md:h-6 text-blue-600 dark:text-blue-400" /></button>
            </div>
        </div>
    );
};
...
const ProfileModal: React.FC<{ user: User; onClose: () => void; onUpdate: () => void }> = ({ user, onClose, onUpdate }) => {
    ...
    const save = () => { if (url) { const u = Database.getUser(); if (u) { u.avatar = url; Database.updateUser(u); onUpdate(); onClose(); } } };
    return (<div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4"><div className="bg-white dark:bg-gray-900 rounded-2xl p-6 w-full max-w-sm text-center border border-gray-100 dark:border-gray-800"><h3 className="font-bold text-lg mb-4 dark:text-white">Update Profile Picture</h3><div className="w-24 h-24 mx-auto mb-4 rounded-full overflow-hidden border-4 border-yellow-400"><AvatarImage src={url} alt="Preview" className="w-full h-full object-cover" /></div><input className="w-full p-2 border rounded mb-4 dark:bg-gray-800 dark:border-gray-700 dark:text-white" placeholder="Image URL..." value={url} onChange={e => setUrl(e.target.value)} /><div className="flex gap-2"><button onClick={onClose} className="flex-1 py-2 border rounded font-bold dark:border-gray-700 dark:text-white">Cancel</button><button onClick={save} className="flex-1 py-2 bg-black dark:bg-white dark:text-black text-white rounded font-bold">Save</button></div></div></div>);
};
...
const JournalModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    return (<div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4"><div className="bg-[#FFFBEB] dark:bg-gray-900 rounded-3xl w-full max-w-2xl h-[600px] flex flex-col shadow-2xl relative border-4 border-white dark:border-gray-800 overflow-hidden"><div className="bg-yellow-400 dark:bg-yellow-600 p-4 md:p-6 flex justify-between items-center"><h2 className="text-xl md:text-2xl font-black text-black flex items-center gap-2"><BookOpen className="w-5 h-5 md:w-6 md:h-6"/> Live Journal</h2><button onClick={onClose} className="p-2 hover:bg-black/10 rounded-full"><X className="w-6 h-6 text-black"/></button></div><div className="flex-1 flex flex-col md:flex-row overflow-hidden"><div className="flex-1 p-4 md:p-6 flex flex-col border-r border-yellow-200 dark:border-gray-800"><textarea className="flex-1 w-full bg-white dark:bg-gray-800 dark:text-white p-4 rounded-xl border border-yellow-200 dark:border-gray-700 focus:border-yellow-500 outline-none resize-none shadow-inner text-sm md:text-base" placeholder="Write your thoughts..." value={content} onChange={e => setContent(e.target.value)}></textarea><button onClick={handleSave} className="mt-4 w-full bg-black dark:bg-white dark:text-black text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">{saved ? <CheckCircle className="w-5 h-5 text-green-500"/> : <Save className="w-5 h-5"/>}{saved ? "Saved" : "Save Entry"}</button></div><div className="w-full md:w-64 bg-white dark:bg-gray-800 p-6 overflow-y-auto hidden md:block"><h3 className="font-bold text-gray-400 text-xs uppercase mb-4 tracking-wider">Previous Entries</h3><div className="space-y-4">{history.length === 0 && <p className="text-gray-400 text-sm">No entries yet.</p>}{history.map(entry => (<div key={entry.id} className="p-3 bg-yellow-50 dark:bg-gray-700 rounded-xl border border-yellow-100 dark:border-gray-600"><p className="text-xs text-gray-400 font-bold mb-1">{new Date(entry.date).toLocaleDateString()}</p><p className="text-sm text-gray-700 dark:text-gray-200 line-clamp-3">{entry.content}</p></div>))}</div></div></div></div></div>);
};
  ...
  return (
    <div className="min-h-screen bg-[#FFFBEB] dark:bg-black font-sans text-gray-900 dark:text-gray-100 selection:bg-yellow-200 transition-colors duration-500 relative overflow-hidden flex flex-col">
      ...
      <nav className="bg-[#FFFBEB]/80 dark:bg-black/80 backdrop-blur-xl border-b border-yellow-100 dark:border-gray-800 sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4 flex justify-between items-center shadow-sm transition-colors relative">
          ...
          {/* CENTERED PANIC BUTTON */}
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              <button 
                  onClick={() => setShowGrounding(true)} 
                  className="bg-yellow-500 hover:bg-yellow-400 text-black px-4 py-2 md:px-6 md:py-2.5 rounded-full font-black text-xs md:text-sm uppercase tracking-widest animate-pulse shadow-xl shadow-yellow-500/20 flex items-center gap-2 hover:scale-105 transition-transform"
              >
                  <LifeBuoy className="w-4 h-4 md:w-5 md:h-5" /> Panic Relief
              </button>
          </div>
