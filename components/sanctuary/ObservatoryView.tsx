...
import { X, Sparkles, Star, Loader2, PlayCircle } from 'lucide-react';
...
const ObservatoryView: React.FC<ObservatoryViewProps> = ({ user, onClose }) => {
    ...
    const [selectedStar, setSelectedStar] = useState<ActivityNode | null>(null);
    const [starPositions, setStarPositions] = useState<{ id: string, type: string, x: number, y: number, r: number, delay: number }[]>([]);
    const [replayIndex, setReplayIndex] = useState<number>(1000);
    ...
    useEffect(() => {
        if (activeTab === 'archive' && activities.length === 0) {
            ...
            Promise.all([
                ...
                jData.forEach(j => {
                    if (j.content.length > 5) arr.push({ id: j.id, type: 'journal', title: 'Journal Entry', content: j.content, date: j.date });
                });

                txData.forEach(tx => {
                    arr.push({ id: tx.id, type: 'transaction', title: 'Dashboard Activity', content: tx.description || 'App Interaction', date: tx.date });
                });
    ...
    }, [activeTab, user.id, activities.length]);
                        ...
                        {isLoadingJournals ? (
                            ...
                            <>
                                ...
                                <svg className="absolute inset-0 w-full h-full pointer-events-none">
                                    ...
                                    {starPositions.slice(0, replayIndex).map((pos, i, arr) => {
                                        if (i === arr.length - 1) return null;
                                        const next = arr[i + 1];
                                        const dist = Math.hypot(next.x - pos.x, next.y - pos.y);
                                        if (dist > 35) return null;
                                        return (
                                            <line key={`pline-${i}-${pos.id}`} x1={`${pos.x}%`} y1={`${pos.y}%`} x2={`${next.x}%`} y2={`${next.y}%`} stroke="#8b5cf6" strokeWidth="1" strokeOpacity="0.35" filter="url(#starGlowArchive)" className="animate-[drawLine_0.5s_ease-out_forwards]" strokeDasharray="100" strokeDashoffset="100" />
                                        );
                                    })}
                                </svg>
                            ...
                            `}</style>
                                ...
                                {starPositions.slice(0, replayIndex).map((pos) => {
                                    const activity = activities.find(a => a.id === pos.id);
                                    if (!activity) return null;

                                    const isSelected = selectedStar?.id === pos.id;
                                    const isTherapy = pos.type === 'therapy';
                                ...
                                {activities.length === 0 && !isLoadingJournals && (
                                    <div className="text-blue-300/50 font-serif italic text-lg z-10">
                                        Your sky is empty. Engage with your dashboard to forge stars.
                                    </div>
                                )}
                                ...
                                {selectedStar && (
                                    <div className="absolute bottom-10 left-10 right-10 md:left-1/2 md:-translate-x-1/2 md:w-[500px] bg-indigo-950/80 backdrop-blur-xl border border-blue-500/30 rounded-2xl p-6 shadow-[0_0_50px_rgba(37,99,235,0.2)] z-50 animate-in slide-in-from-bottom-10 fade-in duration-500">
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            {selectedStar.type === 'therapy' ? (
                                                <Star className="w-5 h-5 text-yellow-400 fill-yellow-400" />
                                            ) : (
                                                <Star className="w-5 h-5 text-purple-400 fill-purple-400" />
                                            )}
                                            <span className="font-sans text-xs uppercase tracking-widest font-bold text-blue-200">
                                                {selectedStar.title} - {new Date(selectedStar.date).toLocaleDateString()}
                                            </span>
                                        </div>
