              ...
              <div className="flex flex-col gap-6 md:gap-8">
                  <div className="flex gap-4 md:gap-6 animate-marquee w-fit px-4">
                      {[...row1, ...row1, ...row1, ...row1].map((spec, i) => (
                          <div key={`${spec.id}-1-${i}`} onClick={() => onLoginClick(true)} className="relative flex-shrink-0 w-44 h-60 md:w-64 md:h-80 bg-white dark:bg-gray-900 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg border border-yellow-100 dark:border-gray-800 hover:scale-105 hover:shadow-2xl hover:border-yellow-400 transition-all duration-300 cursor-pointer group">
          ...
          <div className="max-w-xl mx-auto">
              <div className="bg-[#FFFBEB] dark:bg-gray-900 text-black dark:text-white rounded-[2rem] p-6 text-center relative overflow-hidden shadow-2xl border border-yellow-200 dark:border-gray-800 transition-colors">
                  ...
                  <div className="relative z-10 space-y-3 md:space-y-4">
                      ...
                      <p className="text-black/70 dark:text-white/70 text-xs max-w-sm mx-auto font-medium leading-relaxed">{t('pricing_sub')}</p>
                      <button onClick={() => onLoginClick(true)} className="bg-black dark:bg-white text-white dark:text-black px-6 py-3 rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-gray-800 dark:hover:bg-gray-200 transition-all hover:scale-105 shadow-xl mt-2">{t('pricing_btn')}</button>
                  </div>
              </div>
          </div>
      </section>
          ...
          <div className="max-w-7xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-16 mb-12 md:mb-20">
                  <div className="md:col-span-5 space-y-6 md:space-y-8">
                      <div className="flex items-center gap-3"><div className="w-8 h-8 md:w-10 md:h-10 bg-yellow-400 rounded-xl flex items-center justify-center"><Heart className="w-5 h-5 md:w-6 md:h-6 fill-black text-black" /></div><span className="text-xl md:text-2xl font-black tracking-tight">Peutic</span></div>
                      <p className="text-gray-800 dark:text-gray-500 text-sm md:text-lg leading-relaxed max-w-md">{t('footer_desc')}</p>
                      <div className="flex gap-6">{[Twitter, Instagram, Linkedin].map((Icon, i) => (<button key={i} className="text-gray-800 dark:text-gray-500 hover:text-black dark:hover:text-white transition-colors hover:scale-110 transform"><Icon className="w-5 h-5 md:w-6 md:h-6"/></button>))}</div>
                  </div>
