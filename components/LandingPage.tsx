...
import { Heart, CheckCircle, ArrowRight, Play, Moon, Sun, Megaphone, Twitter, Instagram, Linkedin } from 'lucide-react';
...
const LandingPage: React.FC<LandingPageProps> = ({ onLoginClick }) => {
    ...
    const toggleDarkMode = () => {
        ...
        setDarkMode(newMode);
        localStorage.setItem('peutic_theme', newMode ? 'dark' : 'light');
    };
    ...
    return (
        <div className={`min-h-screen bg-[#FFFBEB] dark:bg-[#0A0A0A] font-sans text-[#0A0A0A] dark:text-[#F3F4F6] selection:bg-yellow-200 selection:text-black transition-colors duration-500 ${lang === 'ar' ? 'rtl' : 'ltr'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
                ...
                <div className="absolute inset-0 bg-[#FFFBEB]/80 dark:bg-black/70 backdrop-blur-[2px]"></div>
                <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#FFFBEB]/40 dark:via-black/40 to-[#FFFBEB] dark:to-[#0A0A0A]"></div>
            </div>
