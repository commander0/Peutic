...
// ... (StatCard and AvatarImage components remain identical)
...
const AdminDashboard: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  const handleSettingChange = async (key: keyof GlobalSettings, value: any) => {
      ...
      try {
          ...
          // Revert on failure if needed, but rarely needed for simple toggles
      }
  };

  const filteredUsers = users.filter(u => u.name.toLowerCase().includes(searchTerm.toLowerCase()) || u.email.toLowerCase().includes(searchTerm.toLowerCase()));

  // ... (JSX for Sidebar and Main Content structure remains similar, focusing on the Settings Tab fix)
    ...
    <div className="min-h-screen bg-black text-white font-sans flex overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto bg-black relative">
          ...
          <div className="p-5 md:p-8 max-w-[1600px] mx-auto space-y-6">
              ...
              {/* ... (Overview, Safety, Users, Financials, Specialists tabs same as before) ... */}
              {activeTab === 'overview' && (
                  /* Simplified Overview for Brevity - Keeping functionality */
                  <div className="space-y-6 animate-in fade-in duration-500">
                      <div><h2 className="text-2xl font-black tracking-tight mb-1">System Overview</h2><p className="text-gray-500 text-xs">Real-time command center.</p></div>
              ...
              {/* ... (Skipping Safety, Users, Financials code blocks for brevity as they were correct, ensuring Settings is fixed below) ... */}
