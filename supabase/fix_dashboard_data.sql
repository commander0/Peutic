...
-- 1. FIX COMPANIONS (Upsert 35 Specialists)
DELETE FROM public.companions; -- Clear duplicates if any
...
-- 2. FIX RLS & ADMIN VISIBILITY
-- Ensure is_admin is secure (bypasses RLS strictly)
...
-- 3. ENSURE USER IS ADMIN (Self-Healing)
-- If the current user has NO role (or is NULL), this query won't help. 
-- But existing users usually have role.
-- We can't elevate the user here without their ID, but policy ensures if they are admin, they see all.
